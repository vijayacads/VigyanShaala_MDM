...
# Step 5: Create log directories (required for osquery and MDM agents to write logs)
$osqueryLogDir = "C:\ProgramData\osquery\logs"
if (-not (Test-Path $osqueryLogDir)) {
    New-Item -ItemType Directory -Path $osqueryLogDir -Force | Out-Null
    Write-Host "Created log directory: $osqueryLogDir" -ForegroundColor Green
} else {
    Write-Host "Log directory exists: $osqueryLogDir" -ForegroundColor Green
}

$mdmLogDir = "C:\ProgramData\VigyanShaala-MDM\logs"
if (-not (Test-Path $mdmLogDir)) {
    New-Item -ItemType Directory -Path $mdmLogDir -Force | Out-Null
    Write-Host "Created MDM log directory: $mdmLogDir" -ForegroundColor Green
} else {
    Write-Host "MDM log directory exists: $mdmLogDir" -ForegroundColor Green
}
...
if ($SupabaseUrl -and $SupabaseKey) {
    ...
    try {
        ...
        try {
            ...
            $notifyInfo = Get-ScheduledTask -TaskName $userNotifyTaskName | Get-ScheduledTaskInfo
            Write-Host "User notification agent started. State=$($notifyInfo.State) LastTaskResult=$($notifyInfo.LastTaskResult)" -ForegroundColor Green
        } catch {
            Write-Host "WARNING: Could not start user notification agent task: $($_.Exception.Message)" -ForegroundColor Yellow
        }
    } catch {
