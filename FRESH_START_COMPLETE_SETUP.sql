...
-- This script DROPS everything first, then recreates it
...
DROP FUNCTION IF EXISTS calculate_performance_status(INTEGER, INTEGER, INTEGER) CASCADE;
DROP FUNCTION IF EXISTS update_device_health_updated_at() CASCADE;
DROP FUNCTION IF EXISTS auto_calculate_performance_status() CASCADE;
DROP FUNCTION IF EXISTS update_location_wifi_mappings_updated_at() CASCADE;
DROP FUNCTION IF EXISTS cleanup_old_chat_messages() CASCADE;
DROP FUNCTION IF EXISTS public.enroll_device(TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, DECIMAL, DECIMAL, TEXT, TEXT, TEXT, TEXT, DATE, TEXT, TEXT, TIMESTAMPTZ, UUID) CASCADE;

-- Drop all tables (in reverse dependency order)
DROP TABLE IF EXISTS user_notifications CASCADE;
DROP TABLE IF EXISTS chat_messages CASCADE;
DROP TABLE IF EXISTS device_commands CASCADE;
DROP TABLE IF EXISTS location_wifi_mappings CASCADE;
...
CREATE INDEX idx_devices_teacher ON devices(assigned_teacher);
CREATE INDEX idx_devices_student_leader ON devices(assigned_student_leader);
CREATE INDEX idx_devices_compliance ON devices(compliance_status);
CREATE INDEX idx_devices_last_seen ON devices(last_seen);
CREATE INDEX idx_devices_hostname ON devices(hostname);
CREATE INDEX idx_devices_inventory_code ON devices(device_inventory_code);
...
CREATE INDEX idx_device_health_performance_status ON device_health(performance_status);
CREATE INDEX idx_device_health_updated_at ON device_health(updated_at);
...
CREATE TABLE location_wifi_mappings (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_location_wifi_mappings_location ON location_wifi_mappings(location_id);
CREATE INDEX idx_location_wifi_mappings_ssid ON location_wifi_mappings(wifi_ssid);
CREATE INDEX idx_location_wifi_mappings_active ON location_wifi_mappings(is_active) WHERE is_active = true;
...
CREATE INDEX idx_user_notifications_device_username_status 
...
-- This matches your old project exactly.
-- =====================================================
